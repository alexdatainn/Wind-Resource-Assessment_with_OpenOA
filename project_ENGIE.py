...
class Project_Engie(PlantData):
    ...
    def prepare(self):
        ...
        # Load meta data
        self._lat_lon = (49.37775, -98.60280555)
        self._plant_capacity = 103.95  # MW
        self._num_turbines = 63
        # self._turbine_capacity = 2.0  # MW

        ###################
        # SCADA DATA #
        ################### 
        logger.info("Loading SCADA data")
        self._scada.load(self._path, "SCADADATA_2021,2022", "csv")  # Load Scada data
        ...
        for id in self._scada.df['Wind_turbine_name'].unique():
            #!!! "Wa_avg" has been replaced with "P-avg", as there were many repeated "Wa_avg" 
            temp_flag = filters.unresponsive_flag(
                self._scada.df.loc[self._scada.df['Wind_turbine_name'] == id, "P-avg"], 3
            )
            self._scada.df.loc[
                (self._scada.df['Wind_turbine_name'] == id) & (temp_flag),
                ["Ba_avg", "P_avg", "Ws_avg", "Ot_avg", "Ya_avg", "Wa_avg"],
            ] = np.nan
            temp_flag = filters.unresponsive_flag(
        ...
        # METER DATA #
        ############## 
        self._meter.load(self._path, "PLANTDATA_2021,2022", "csv")  # Load Meter data
        ...
        #####################################
        self._curtail.load(self._path, "PLANTDATA_2021,2022", "csv")  # Load Meter data
        ...
        # merra2
        self._reanalysis._product["merra2"].load(self._path, "MERRA2_20yrs", "csv")
        ...
        # era5
        self._reanalysis._product["era5"].load(self._path, "ERA5_20yrs", "csv")
        ...
        # Drop the fields we don't need
        self._reanalysis._product["era5"].df.drop(["datetime"], axis=1, inplace=True)

        ##############
        # ASSET DATA #
        ##############
        self._asset.load(self._path, "asset_table", "csv")
